...
import com.kdu.caching.Application;
...
//
    ...
    void testGetReverseGeoCode() {
        try {
            ...
            int resultCode = result.getResponse().getStatus();
            assertTrue(resultCode >= HttpStatus.OK.value() && resultCode < HttpStatus.MULTIPLE_CHOICES.value(),
                    "HTTP Status Code should be 2xx for successful response");

            String resultInStringFormat = result.getResponse().getContentAsString();
            assertTrue(resultInStringFormat.contains("\"name\":\"16192 W Delhi Ave\""),
                    "Expected name in the response body. Actual response: " + resultInStringFormat);
