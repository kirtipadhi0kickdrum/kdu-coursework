...
//    }
    ...
    public static void addUserToHouseByNonAdmin(MockMvc mockMvc, String... args) throws Exception {
        try {
            ...
            // Fetch house ID by name
            String houseName = "user1";  // Replace with the actual houseName
            String houseId = mockMvc.perform(MockMvcRequestBuilders.get("/api/v1/house/{houseName}/id", houseName))
            ...
            mockMvc.perform(MockMvcRequestBuilders.post("/api/v1/house/{houseId}/add-user", houseId)
                            ...
                            .header("Authorization", "Bearer " + userToken)
                            .contentType(MediaType.APPLICATION_JSON))
                    .andExpect(MockMvcResultMatchers.status().isBadRequest())  // Change to expect 400
                    .andDo(result -> {
                        System.out.println("addUserToHouseByNonAdmin TEST PASSED");
                    });
